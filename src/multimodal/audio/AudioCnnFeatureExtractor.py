...
class AudioCnnFeatureExtractor(CnnFeatureExtractorFather):
    ...
    def set_model(self, model_name):
        ...
        if 'torch' in self._framework_list or 'torchaudio' in self._framework_list:
            ...
            self._model = Wav2Vec2FeatureExtractor.from_pretrained(model_name)
    ...
    def extract_feature(self, sample_input):
        ...
        if 'transformers' in self._framework_list:
            ...
            '''
            # Audio Resample
            pre_processor = torchaudio.transforms.Resample(sample_rate, 16000)
            resempled_audio = pre_processor(audio)
            # Preprocessa l'audio
            # inputs = self._model(resempled_audio, return_tensors="pt")

            # feature extraction con modello
            modello = Wav2Vec2Model.from_pretrained(self._model_name)
            outputs = modello(resempled_audio, output_hidden_states=True)
